...
導入後に **AWS アカウントやロールが増減したとき**は、`plan` → `apply` を再実行するだけです（冪等。新規は `added`、削除は `removed` として検出）。手順と注意点は[導入後の定常運用](docs/setup-guide.md#導入後の定常運用アカウント--ロールが増減したとき)を参照（`--cache` 常用時は `--refresh-cache` を併用）。
